...
navigate: function(dir) {
  if (imce.vars.navbusy || (dir == imce.conf.dir)) return;

SwalConfirm.fire({text:"You want to navitate to this folder?"}).then(function(){
     if(confirm.value){
...
fopValidate: function(fop) {
  ...
  switch (fop) {
    ...
    SwalConfirm.fire({text:"You want to delete selected files?"}).then(function(confirm){
        if(confirm.value){
            deleter = true;
        }else{
            deleter = false;
...
setSendTo: function (title, func) {
  ...
  var opFunc = function () {
    if (imce.selcount != 1){
        SwalInfo.fire({text:"Please select a file..."});
...
validateSelCount: function (Min, Max) {
  if (Min && imce.selcount < Min) {
      if(Min == 1){
          ...
          SwalInfo.fire({text:"You must to select minimum amount of files..."})
      }
